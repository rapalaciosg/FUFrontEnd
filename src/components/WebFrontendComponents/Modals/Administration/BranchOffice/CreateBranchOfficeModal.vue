...
export default {
  ...
  props: {},
  emits: ["branch-office-created", "close-modal"],
  data() {
    return {};
  },
  setup(props, { emit }) {
    ...
    const { mutate: createBranchOffice } = useMutation(CREATE_BRANCH_OFFICE, () => ({ variables: { inputModel: branchOffice } }));
    ...
    const onSubmit = handleSubmit(async (values, actions) => {
      ...
      await createBranchOffice()
        .then((response) => {
          if (response.data.createBranchOffice.statusCode === "OK") toast.success("Sucursal creada exitosamente", { timeout: 2000 });
          else toast.error(response.data.createBranchOffice.message, { timeout: 2000 });
