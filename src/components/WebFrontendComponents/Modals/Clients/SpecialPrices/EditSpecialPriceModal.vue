  ...
  setup(props, { emit }) {
    ...
    watch(
      ...
      (newValue) => {
        ...
        productId.value = findSelectValues(
          productsFormatted,
          newValue.product.productId
        );
        branchOfficeId.value = findSelectValues(
          branchOfficesFormatted,
          newValue.branchOffice.branchOfficeId
        );
        customerId.value = findSelectValues(
          customersFormatted,
          newValue.customer.customerId
        );
      },
      { deep: true }
    );
    ...
    const { mutate: updateSpecialPrice } = useMutation(UPDATE_SPECIAL_PRICE, () => ({
    ...
    const onSubmit = handleSubmit((values, actions) => {
      ...
      updateSpecialPrice()
        .then((response) => {
          ...
          toast.success("Precio especial actualizado exitosamente", {
            timeout: 2000,
          });
        })
        .catch((error) => {
