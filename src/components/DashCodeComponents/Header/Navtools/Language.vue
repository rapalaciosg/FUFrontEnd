    ...
    <Listbox v-model="selectLanguage">
      <div class="relative z-[22]">
        ...
        >
          ...
          /></span>
          <span
            class="text-sm md:block hidden font-medium text-white dark:text-slate-300"
        ...
        >
          <ListboxOptions
            class="absolute min-w-[100px] ltr:right-0 rtl:left-0 md:top-[50px] top-[38px] w-auto max-h-60 overflow-auto border border-slate-200 dark:border-slate-700 rounded bg-secondaryBase dark:bg-slate-800 mt-1"
          >
            <ListboxOption
              v-slot="{ active }"
            ...
            >
              <li
                :class="[
                  active
                    ? 'bg-slate-100 dark:bg-slate-700 dark:bg-opacity-70 bg-opacity-10 dark:text-white '
                    : 'text-white dark:text-slate-300',
                  'w-full border-b text-white border-b-gray-500 border-opacity-10 px-2 py-2 last:border-none last:mb-0 cursor-pointer first:rounded-t last:rounded-b',
                ]"
              >
